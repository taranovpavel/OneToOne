...
const GetCategories=({action, isSearch})=>{return <Container classname={"flex-wrap"} inner={<ItemCardPage type={action} isSearch={isSearch}/>}/>}

const MainPage = () => {
    const {isRUS,tab} = useSelector(state => state.itemsReducer)
    ...
    return (
        <>
            <Header isFixed={true} setInput={setInput} isSearch={true}/>
            <Container inner={
                <>
